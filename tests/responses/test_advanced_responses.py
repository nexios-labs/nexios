...
from nexios.http.response import BaseResponse, PlainTextResponse, JSONResponse, HTMLResponse
...
def test_custom_json_response_class(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_custom_html_response_class(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_paginate_response(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test pagination with page number strategy"""
    app = NexiosApp()
    
    @app.get("/paginated")
    async def paginated_data(request: Request, response: Response):
        items = [{"id": i, "name": f"Item {i}"} for i in range(1, 101)]
        return response.paginate(items, strategy="page_number", page_size=10)
    
    with test_client_factory(app) as client:
        resp = client.get("/paginated")
        assert resp.status_code == 200
        data = resp.json()
        # Check pagination structure
        assert "results" in data or "items" in data or "data" in data


def test_paginate_with_page_param(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test pagination with page parameter"""
    app = NexiosApp()
    
    @app.get("/paginated-page")
    async def paginated_page(request: Request, response: Response):
        items = [{"id": i, "value": i * 10} for i in range(1, 51)]
        return response.paginate(items, strategy="page_number", page_size=10)
    
    with test_client_factory(app) as client:
        # Get first page
        resp1 = client.get("/paginated-page?page=1")
        assert resp1.status_code == 200
        
        # Get second page
        resp2 = client.get("/paginated-page?page=2")
        assert resp2.status_code == 200


def test_paginate_limit_offset(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test pagination with limit-offset strategy"""
    app = NexiosApp()
    
    @app.get("/paginated-offset")
    async def paginated_offset(request: Request, response: Response):
        items = [{"id": i, "name": f"User {i}"} for i in range(1, 101)]
        return response.paginate(items, strategy="limit_offset", limit=20)
    
    with test_client_factory(app) as client:
        resp = client.get("/paginated-offset")
        assert resp.status_code == 200


def test_paginate_cursor(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test pagination with cursor strategy"""
    app = NexiosApp()
    
    @app.get("/paginated-cursor")
    async def paginated_cursor(request: Request, response: Response):
        items = [{"id": i, "timestamp": i * 1000} for i in range(1, 51)]
        return response.paginate(items, strategy="cursor", page_size=10)
    
    with test_client_factory(app) as client:
        resp = client.get("/paginated-cursor")
        assert resp.status_code == 200


def test_paginate_empty_list(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test pagination with empty list"""
    app = NexiosApp()
    
    @app.get("/paginated-empty")
    async def paginated_empty(request: Request, response: Response):
        items: List[Dict[str, Any]] = []
        return response.paginate(items, strategy="page_number", page_size=10)
    
    with test_client_factory(app) as client:
        resp = client.get("/paginated-empty")
        assert resp.status_code == 200


def test_paginate_single_page(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test pagination with data that fits in single page"""
    app = NexiosApp()
    
    @app.get("/paginated-single")
    async def paginated_single(request: Request, response: Response):
        items = [{"id": i} for i in range(1, 6)]
        return response.paginate(items, strategy="page_number", page_size=10)
    
    with test_client_factory(app) as client:
        resp = client.get("/paginated-single")
        assert resp.status_code == 200


# ========== Async Pagination Tests ==========

def test_apaginate_response(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test async pagination"""
    app = NexiosApp()
    
    @app.get("/apaginated")
    async def apaginated_data(request: Request, response: Response):
        items = [{"id": i, "name": f"Item {i}"} for i in range(1, 101)]
        return await response.apaginate(items, strategy="page_number", page_size=10)
    
    with test_client_factory(app) as client:
        resp = client.get("/apaginated")
        assert resp.status_code == 200


def test_apaginate_limit_offset(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test async pagination with limit-offset"""
    app = NexiosApp()
    
    @app.get("/apaginated-offset")
    async def apaginated_offset(request: Request, response: Response):
        items = [{"id": i} for i in range(1, 51)]
        return await response.apaginate(items, strategy="limit_offset", limit=15)
    
    with test_client_factory(app) as client:
        resp = client.get("/apaginated-offset")
        assert resp.status_code == 200
...
def test_response_with_empty_string(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_response_ensure_ascii_false(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_response_ensure_ascii_true(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_conditional_response_type(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_response_based_on_method(test_client_factory: Callable[[NexiosApp], TestClient]):
