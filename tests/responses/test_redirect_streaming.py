...
def test_redirect_308_permanent_redirect(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_redirect_with_query_params(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_streaming_response_bytes(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_streaming_response_with_content_type(test_client_factory: Callable[[NexiosApp], TestClient]):
    ...
    async def stream_json(request: Request, response: Response):
        return response.stream(generate_json_lines(), content_type="application/x-ndjson")
...
def test_streaming_response_with_status_code(test_client_factory: Callable[[NexiosApp], TestClient]):
