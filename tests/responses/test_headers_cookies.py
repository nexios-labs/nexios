...
def test_override_header(test_client_factory: Callable[[NexiosApp], TestClient]):
    ...
    async def override_header(request: Request, response: Response):
        return (response
                .set_header("X-Value", "first", overide=True)
...
def test_set_cookie_with_max_age(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_headers_and_cookies_together(test_client_factory: Callable[[NexiosApp], TestClient]):
