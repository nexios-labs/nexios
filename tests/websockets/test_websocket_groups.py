    ...
    async def on_receive(self, websocket: WebSocket, data: Any) -> None:
        ...
        if action == "join":
            ...
            for group in self.joined_groups:
                await self.broadcast(
                    payload={"message": message, "from_group": group},
                    group_name=group
                )
            await websocket.send_json({
                "action": "broadcast_complete",
                "groups": len(self.joined_groups)
            })
...
def test_multiple_groups_per_consumer(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_channel_cleanup_on_disconnect(test_client_factory: Callable[[NexiosApp], TestClient]):
