...
def test_app_level_middleware_basic(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_app_level_middleware_modifies_request(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test middleware that modifies request"""
    app = NexiosApp()
    
    async def add_custom_header_middleware(request: Request, response: Response, call_next):
...
def test_app_level_middleware_modifies_response(test_client_factory: Callable[[NexiosApp], TestClient]):
    """Test middleware that modifies response"""
    app = NexiosApp()
    
    async def add_response_header_middleware(request: Request, response: Response, call_next):
        await call_next()
...
def test_app_level_middleware_multiple(test_client_factory: Callable[[NexiosApp], TestClient]):
    ...
    with test_client_factory(app) as client:
        ...
        assert execution_order == [
            "m3_before", "m2_before", "m1_before",
            "handler",
            "m1_after", "m2_after", "m3_after"
        ]


def test_app_level_middleware_early_return(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_app_level_middleware_exception_handling(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_app_level_middleware_applies_to_all_routes(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_base_middleware_with_config(test_client_factory: Callable[[NexiosApp], TestClient]):
...
def test_middleware_request_state(test_client_factory: Callable[[NexiosApp], TestClient]):
