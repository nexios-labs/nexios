...
def plot_results(df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'{plots_dir}/benchmark_results_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def analyze_results():
    ...
    for result_file in glob.glob(f'{results_dir}/*.json'):
        ...
        with open(result_file, 'r') as f:
            ...
            total_response_time = sum(endpoint['total_response_time'] for endpoint in data)
            
            # Calculate average RPS across all endpoints
            all_reqs_per_sec = {}
            for endpoint in data:
                for timestamp, count in endpoint['num_reqs_per_sec'].items():
                    all_reqs_per_sec[timestamp] = all_reqs_per_sec.get(timestamp, 0) + count
