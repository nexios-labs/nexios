...
def template_engine():
    config = TemplateConfig(
        ...
        custom_filters={
            "currency": lambda value, symbol="$": f"{symbol}{value:,.2f}"
        }
    )
...
def app():
    ...
    app.add_middleware(TemplateContextMiddleware(
        default_context={"app_name": "Test App"},
        context_processor=user_context
    ))
    
    @app.get("/")
    async def home(request, response):
        return await render("welcome.html", {
            "name": "Test User",
            "message": "Welcome to our test app!"
        })
...
async def test_template_inheritance(template_engine):
    ...
    content = await template_engine.render(
        "welcome.html",
        {"name": "User", "message": "Test message"}
    )
