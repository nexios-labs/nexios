...
async def test_invalid_boundary():
    ...
    headers = Headers(raw=[
        (b"content-type", b"multipart/form-data; boundary=" + boundary),  # Different boundary
        (b"content-length", str(len(form_data)).encode())
    ])
...
async def test_multiple_file_uploads(client):
    ...
    data = {
        "description": "Multiple files upload test"
    }
...
async def test_unicode_filename_and_content(client):
    ...
    files = {
        "unicode_file": (filename, content, "text/plain; charset=utf-8")
    }
...
async def test_binary_file_upload(client):
    ...
    files = {
        "binary_file": ("data.bin", binary_content, "application/octet-stream")
    }
