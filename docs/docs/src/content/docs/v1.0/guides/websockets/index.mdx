...
A `Hub` keeps its room registry in ordinary instance state — a dictionary in
one process's memory. It is not shared between processes, workers, or machines.
...
**A Redis or NATS pub/sub fan-out layer.** Every worker subscribes to the rooms
its local connections care about, and `broadcast` publishes to the bus instead
of iterating a local dict. This is what Django Channels' channel layer does.
Note that a [`Backlog`](/packages/wire/backlog/) alone is not enough: it gives
a joining peer its replay, but it does not carry a live broadcast to peers
connected to another process — that needs a transport as well.
